...
def calc_linear_coefs(x, y):
    ...
    if denominator == 0:
        raise Exception("Невозможно посчитать коэффициенты: деление на ноль")
...
def calc_exp_coefs(x, y):
    if np.any(y <= 0):
        raise Exception("Невозможно посчитать коэффициенты, y <= 0")
...
def calc_pow_coefs(x, y):
    if np.any(x <= 0) or np.any(y <= 0):
        raise Exception("Невозможно посчитать коэффициенты, x <= 0 или  y <= 0")
...
def calc_log_coefs(x, y):
    if np.any(x <= 0):
        raise Exception("Невозможно посчитать коэффициенты, x <= 0")
...
def exclude_noise(func, x, y):
    ...
    x_del = np.delete(x, index)
    y_del = np.delete(y, index)
    return x_del, y_del


def left_dots(func, x, y):
    index = np.argmax((func(x) - y) ** 2)
    x_del = np.delete(x, index)
    y_del = np.delete(y, index)
    return x_del, y_del
...
class LeastSquares(object):
    ...
    def recalc_coefs(self):
        x1, y1 = left_dots(self.function, self.x, self.y)
    ...
    def draw_graphics(self):
        fig, ax = plt.subplots()
        ax.grid()
        ax.scatter(self.x, self.y, label="exclusion point", c="r")
        b = max(self.x)
        a = min(self.x)
        step = (b - a) / 10000
        ...
        if len(self.function.recalc_coefs) != 0:
            ax.plot(x, self.function(x, recalc_use=True),
                    label="approximation after exclude: {0}".format(self.function.get_description(True)))
            ax.scatter(self.x1, self.y1, label="source", c="g")
